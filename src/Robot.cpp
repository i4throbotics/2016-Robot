...
void Robot::RobotInit() {
	...
	SmartDashboard::PutNumber("Register Adress",0);
	/**SmartDashboard::PutNumber("Auto Drive Speed",	0.5);
...
void Robot::TeleopPeriodic() {
	...
	if(x<std::pow(2,9)){
		if(!Robot::wire->Read(x,sizeof(double),&buffer[0])){
			std::memcpy(&angle,&buffer[0],sizeof(double));
			file<<angle<<"\t"<<x<<"\n";
		}

		x++;
	}
	SmartDashboard::PutNumber("Data",angle);
